...
const enhancedPropertyFormSchema = z.object({
  ...
  description: z.string().min(50, "Description must be at least 50 characters to provide adequate details"),
  propertyType: z.string().min(1, "Property type is required"),
  listingType: z.enum(["rent", "sale"]),
  price: z.string().min(1, "Price is required"),
  bedrooms: z.number().min(0).max(20),
  bathrooms: z.number().min(1).max(20),
  squareFeet: z.number().min(100, "Minimum 100 sq ft required").optional(),
  address: z.string().min(10, "Complete address is required"),
  city: z.string().min(2, "City is required"),
  state: z.string().min(2, "State is required"),
  postalCode: z.string().min(5, "Valid postal code required"),
  latitude: z.string().min(1, "Property location coordinates are required"),
  longitude: z.string().min(1, "Property location coordinates are required"),
  landSize: z.number().min(1, "Land size must be greater than 0").optional(),
  propertyCondition: z.enum(["excellent", "good", "fair", "needs_renovation"]),
  agentLicense: z.string().min(5, "Valid agent license is required"),
  distanceToLRT: z.string().optional(),
  distanceToMall: z.string().optional(),
